...
## 3.1 - Model Training (optional)

## 3.2 - Setting up the Model (optional)

# 4 - Application
When there is no need for any model training or setting up a new model, the pre-trained model can be directly applied to the preprocessed data. The function `classify_breeding_behaviour` takes the preprocessed data and the metadata information as input and returns a data frame with the original data and an additional column indicating the classified breeding behaviour state for each time point.
```{r}
